...
class Fila:
    ...
    def combina(self, f1, f2):
        while not f1.estaVazia() and not f2.estaVazia():
            if not f1.estaVazia():
                no1 = No(f1.desenfileirar())
            else:
                no1 = None

            if not f2.estaVazia():
                no2 = No(f2.desenfileirar())
            else:
                no2 = None

            if no1 != None:
                if self.estaVazia():
                    self.__head.inicio = self.__head.fim = no1
                else:
                    self.__head.fim.prox = no1
                    self.__head.fim = no1
                self.__head.tamanho += 1

            if no2 != None:
                if self.estaVazia():
                    self.__head.inicio = self.__head.fim = no2
                else:
                    self.__head.fim.prox = no2
                    self.__head.fim = no2
                self.__head.tamanho += 1
